...
pipeline {
    environment {
        ...
        SSH_USER = "ubuntu"
        STAGING_IP = "44.201.180.81"
        PROD_IP = "44.201.180.81"
    }
    agent any
    stages {
        // stage('Build image') {
                ...
                script {
                    sshagent(credentials: ['private_key']) {
                        sh '''
                            ...
                            ssh -o StrictHostKeyChecking=no $SSH_USER@$STAGING_IP "docker run --rm --name $IMAGE_NAME -d -p $APP_EXPOSED_PORT:$INTERNAL_PORT ${DOCKERHUB_USR}/$IMAGE_NAME:$IMAGE_TAG"
                            sleep 10
